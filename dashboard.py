...
from bokeh.models import (
    ...
    FixedTicker, Label, LinearAxis, Range1d, CheckboxGroup
)
...
source_monthly_view = ColumnDataSource(df_monthly_filt)
...
monthly_agg_callback = CustomJS(args=dict(
    s_models_view=source_models_view, s_view=source_monthly_view,
    selector=monthly_agg_select, p2=p2
), code="""
    const aggMethod = selector.value;
    const v_data = s_models_view.data;
    const monthlyMap = {};
    for (let i = 0; i < v_data['pub_date_ms'].length; i++) {
        const d = new Date(v_data['pub_date_ms'][i]);
        ...
        const lp = v_data['log_params'][i] || 0;
        const nf = v_data['negative_log_flops'][i] || 0;
    ...
    s_view.data = monthData;
    s_view.change.emit();

    // Reset y-range to auto-scale after change
    p2.y_range.start = null;
    p2.y_range.end = null;
""")
monthly_agg_select.js_on_change('value', monthly_agg_callback)
chart2_header = row(monthly_agg_label, monthly_agg_select, margin=(0, 0, 4, 0))
chart2_block = column(chart2_header, p2)
...
chart3_header = row(chart3_theme_label, chart3_theme_select, margin=(0, 0, 4, 0))
chart3_block = column(chart3_header, p3)
...
med_mw = df_company_view['capacity_mw'].median() if not df_company_view.empty else 1000
med_carbon = df_company_view['carbon_intensity'].median() if not df_company_view.empty else 350

p4 = figure(
    title="Compute Capacity vs. Grid Carbon Intensity",
    x_axis_label="Total Capacity (MW)",
...
hline4 = Span(location=med_carbon, dimension='width', line_color='#484f58', line_width=1.5, line_dash='dashed')
p4.add_layout(vline4)
p4.add_layout(hline4)
# Color by carbon intensity
color_mapper_4 = LinearColorMapper(palette=list(reversed(Turbo256)), low=100, high=600)
scatter4 = p4.scatter(
...
color_bar_4 = ColorBar(
    ...
    title="Carbon Intensity (gCO\u2082/kWh) \u2192 greener = cleaner",
...
chart5_sort_callback = CustomJS(args=dict(
    source=source_co2_country,
    p5_y_range=p5.y_range,
    p5_x_range=p5.x_range,
    color_mapper=color_mapper_co2,
    selector=chart5_sort_select,
    x_axis=p5.xaxis[0]
), code="""
    const field = selector.value;
    const data = source.data;
    const n = data['country'].length;

    // Sort index ascending (so biggest value ends up at TOP of hbar)
    let idx = Array.from({length: n}, (_, i) => i);
    idx.sort((a, b) => data[field][a] - data[field][b]); // ascending = top of hbar is highest

    // Reorder ALL arrays by sorted index
    const new_data = {};
    for (const key in data) {
        new_data[key] = idx.map(i => data[key][i]);
    }

    // KEY FIX: set display_value to the ACTIVE sort field values
    new_data['display_value'] = new_data[field].slice();

    source.data = new_data;
    source.change.emit();

    // Update y_range so bar ordering matches sort
    p5_y_range.factors = new_data['country'].slice();

    // Reset x_range to fit the new display values
    const maxVal = Math.max(...new_data['display_value']);
    p5_x_range.start = 0;
    p5_x_range.end = maxVal * 1.08;

    // Update color mapper range to match the new metric scale
    color_mapper.low = 0;
    ...
    x_axis.axis_label = labels[field] || '';
""")
chart5_sort_select.js_on_change('value', chart5_sort_callback)
chart5_header = row(chart5_sort_label, chart5_sort_select, margin=(0, 0, 4, 0))
chart5_block = column(chart5_header, p5)
...
chart6_header = row(chart6_toggle_label, chart6_toggle, margin=(0, 0, 4, 0))
chart6_block = column(chart6_header, p6)
...
matrix_toggle = Select(
    title="Chart 4 Aggregation:", options=["By Company", "By Country"],
    value="By Company", width=240, margin=(0, 0, 2, 20)
)
...
global_filter_callback = CustomJS(args=dict(
    ...
    s_chart1=source_chart1, s_monthly_view=source_monthly_view,
    monthly_sel=monthly_agg_select,
    date_sl=date_slider, param_sl=param_slider,
    countries=country_select, access=access_filter,
    p1_y_range=p1.y_range, p2=p2
), code="""
    ...
    for (let i = 0; i < m_data['pub_date_ms'].length; i++) {
        ...
        if (pass_date && pass_param && pass_ctry && pass_acc) {
            ...
            let date_ms = m_data['pub_date_ms'][i];
            let model_name = m_data['Model'][i];
    ...
    // Reset y-range of p2 to auto-scale
    p2.y_range.start = null;
    p2.y_range.end = null;
""")
...
matrix_toggle_callback = CustomJS(args=dict(
    ...
    // Reset x and y ranges of p4 to auto-scale
    p4.x_range.start = null;
    p4.x_range.end = null;
    p4.y_range.start = null;
    p4.y_range.end = null;
""")
...
narrative_4 = make_narrative(
    "04",
    "The Dilemma: Large Capacity vs. Dirty Grids",
    "The quadrant plot maps compute capacity against carbon intensity to identify emissions contributions. The bottom-right quadrant shows clean energy usage, while the top-right indicates high carbon risk.",
    "Not all compute is equal &mdash; an entity's position on this quadrant determines whether its AI growth is green or environmentally damaging."
)
